...
- Reuse the existing active-tenant and effective Project authorization path. `projects.read` is required first; `systems.read` controls list/detail and `systems.manage` controls create/update.
- Extend configurable PROJECT-role delegation so an authorized organization-level role administrator can assign `projects.read`, `systems.read`, and `systems.manage` through existing `ProjectAccess`; no parallel authorization mechanism is introduced.
- Define stable validation, conflict, not-found, and authorization behavior that does not disclose cross-tenant or project-mismatched resources, including `SYSTEM_CRITICALITY_INVALID` for unsupported criticality values.
...
- **Authorization/security:** implementation will compose `ProjectAuthorizationService` with the active organization and existing `ProjectAccess`, requiring `projects.read` before the operation-specific catalogued `systems.*` permission; resource IDs alone never authorize access.
...
- **Risk:** incorrect ordering of tenant lookup and permission checks could disclose a foreign Project/System, and an incomplete Project-role allowlist would make the required ProjectAccess-derived `projects.read` plus `systems.*` grants impossible. The design and tests make both boundaries explicit.
