...
#### Scenario: Reject unsupported criticality
- **GIVEN** a caller submits a well-formed `criticality` value outside `LOW`, `MEDIUM`, `HIGH`, and `MISSION_CRITICAL`
- **WHEN** the create request is validated
- **THEN** it returns `400 SYSTEM_CRITICALITY_INVALID` and creates no system

#### Scenario: Reject malformed or forbidden create input
- **GIVEN** a caller submits an invalid code, malformed type or shape, or any ownership, timestamp, deletion, or JSON metadata field
- **WHEN** the create request is validated
...
`PATCH /api/projects/:projectId/systems/:systemId` SHALL accept only `name`, `description`, `businessOwner`, `technicalOwner`, `criticality`, and `businessDomain`, apply the same applicable field validation as creation, and preserve `id`, `projectId`, `code`, `createdAt`, and all JSON metadata. A well-formed criticality value outside `LOW`, `MEDIUM`, `HIGH`, and `MISSION_CRITICAL` SHALL return `400 SYSTEM_CRITICALITY_INVALID`; malformed type/shape and forbidden fields SHALL return `400 VALIDATION_ERROR`. This capability SHALL not move systems between Projects, archive them, or hard-delete them.

#### Scenario: Update only mutable functional metadata
- **GIVEN** a caller has effective `projects.read` and `systems.manage` on the Project
...
Every route SHALL require an authenticated active tenant. It SHALL first resolve the route Project only where `Project.id = projectId`, `Project.organizationId = activeOrganizationId`, and `Project.deletedAt IS NULL`; missing or foreign Projects SHALL return `404 PROJECT_NOT_FOUND`. For an in-tenant Project, it SHALL resolve the caller's existing effective Project permissions as the deduplicated union of valid ORGANIZATION-role permissions and valid matching `ProjectAccess` PROJECT-role permissions, then require `projects.read`; a caller missing it SHALL receive `403 PROJECT_ACCESS_DENIED`. Only then SHALL it require `systems.read` for list/detail or `systems.manage` for create/update; a caller missing the operation's `systems.*` permission SHALL receive `403 SYSTEM_ACCESS_DENIED`. The authorization path SHALL not hardcode role names or introduce a parallel permission mechanism.

#### Scenario: ProjectAccess grants Project and system access
- **GIVEN** an ACTIVE member has no organization-level `projects.read` or `systems.read` but has a same-tenant PROJECT role assigned through `ProjectAccess` with `projects.read` and `systems.read`
- **WHEN** the member lists or reads systems for that assigned Project
- **THEN** the request succeeds without a hardcoded role name

#### Scenario: System permission alone does not grant Project access
- **GIVEN** an ACTIVE member has a same-tenant PROJECT role assigned through `ProjectAccess` with `systems.read` but without `projects.read`
- **WHEN** the member lists or reads systems for that assigned Project
- **THEN** the response is `403 PROJECT_ACCESS_DENIED`
...
- **GIVEN** a caller reaches an in-tenant Project with effective `projects.read` but lacks the operation's required `systems.*` permission
...
Swagger SHALL document all four routes, UUID parameters, request and response DTOs, `SystemCriticality`, mutable versus immutable fields, `projects.read` then `systems.read`/`systems.manage`, Project ownership, tenant behavior, and every documented success or stable error status, including `SYSTEM_CRITICALITY_INVALID`, `VALIDATION_ERROR`, `PROJECT_ACCESS_DENIED`, and `SYSTEM_ACCESS_DENIED`. Unit tests SHALL cover normalization, unsupported-criticality mapping, permission composition, duplicate mapping, immutable updates, and service-scoped lookup. PostgreSQL E2E tests SHALL cover create/list/detail/update, same-Project duplicate rejection and cross-Project acceptance, unsupported criticality, malformed/forbidden input, PROJECT-role-derived `projects.read` plus `systems.*` permissions, `systems.*` without `projects.read`, missing system permissions, cross-tenant list/create/read/update, Project/System mismatch, no metadata exposure, Swagger, and regression of Projects/Auth/RBAC.
...
- **THEN** its documented routes, ordered permissions, fields, enum values, ownership, and errors match this specification
