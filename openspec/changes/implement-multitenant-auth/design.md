...
`JwtAuthGuard` lee `Authorization: Bearer`, verifica la firma JWT, carga la `UserSession` por `sid`, y comprueba `revokedAt=null`, `expiresAt>now`. Si `org` está presente en el payload, verifica `OrganizationMembership ACTIVE` para ese usuario y organización. Devuelve `SESSION_REVOKED` / `SESSION_EXPIRED` / `ORGANIZATION_ACCESS_DENIED`. Expone el contexto (user, session, activeOrganizationId, membership, roles) vía decoradores `@CurrentUser()` / `@CurrentTenant()`. Aplica a `me`, `select-organization`, `refresh`, `logout`. Esto materializa la regla multi-tenant de AGENTS.md (nunca autorizar por UUID solo).
...
- [Riesgo] Sesión con `organizationId=null` accede a recursos tenant-scoped → Mitigación: el guard exige membership ACTIVE cuando `org` está presente; sin org, solo endpoints de selección.
